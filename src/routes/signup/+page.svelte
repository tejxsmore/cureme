...
<div class="flex justify-center items-center bg-dark min-h-screen p-4">
	<form on:submit={handleSubmit} class="flex flex-col bg-white text-black p-8 rounded-3xl sm:max-w-sm w-full">
		...
		<p class="font-light text-black pb-6">to register to Commune</p>

		<label for="name" class="font-normal pb-2">Name</label>
		<input type="text" id="name" name="name" required class="focus:outline-none p-2 mb-6 w-full font-normal rounded-lg border-[1px] border-grey" />
		...
		/>
		<button type="submit" class="bg-red text-white font-semibold p-2 rounded-lg border-[1px] border-red hover:bg-rose-600 mb-6">Signup</button>
		<p class="text-md font-light">Already have an account? <a href="/login" class="font-semibold">Login</a></p>
		{#if formError}
